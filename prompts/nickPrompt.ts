...
- **The excact place from stack trace #{step} starts at symbols:**
\`\`\`javascript
{codePlace}
\`\`\`
`;
...
export const nextNickPrompt = `
${generalStates}

Your task is to analyze the following production error, which includes a stack trace and a snippet from a minified JavaScript file. Your goal is to identify the exact location (file path, line number, and function) in the original, unminified source code of stack trace #{step} where the error occurred in stack trace #1. Found code should call the code that we found on the previous step.
